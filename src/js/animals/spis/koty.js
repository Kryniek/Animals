app.controller('kotySpisCtrl', function($scope){
    ...
    function getValuesJson(){
        $.getJSON("../../json/kot.json", function(data){
            for(var i=0; i<animalsCount; i++){
                ...
                document.getElementById("path"+i).src = "../../img/animals/cats/" + data.cats[i].ElementId + ".jpg";
                document.getElementById("race"+i).textContent = data.cats[i].Race;
    ...
    function getJSONSecondView(){
        var imgCount=0;
        $.getJSON("../../json/kot.json", function(data){
            imgCount=data.cats[$scope.jsonId].MaxPhotos;
            if(imgCount>40) imgCount=40;
            for(var i=1; i<=imgCount; i++)
                {
                    document.getElementById("firstPath"+i).src = "../../img/animals/cats/" + data.cats[$scope.jsonId].ElementId + "/" + i + ".jpg";
                }
            
            document.getElementById("firstRace").textContent = data.cats[$scope.jsonId].Race;
            
            document.getElementById("firstDescription").textContent = data.cats[$scope.jsonId].Description;
            
            document.getElementById("firstDescription2").textContent = data.cats[$scope.jsonId].Description2;
        });
    };
